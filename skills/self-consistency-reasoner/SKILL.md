...
description: >
  Internal reasoning technique invoked by systematic-debugging,
  systematic-problem-solving, and verification-before-completion for
  high-stakes multi-step inference. Generates N independent reasoning
  paths and takes majority vote to surface confident-but-wrong
  single-chain failures. DO NOT invoke independently — this skill is
  embedded in the skills that need it.
---
...
- **systematic-problem-solving** — during diagnosis and option evaluation
...
- Being wrong has real cost (wrong diagnosis wastes edits; false "done" wastes review cycles; wrong decision wastes resources)
...
| Business problem diagnosis (why did metric X drop?) | 5 paths |
| High-stakes decision with multiple competing options | 7 paths |
...
- For business diagnosis: start from different causal chains (demand-side, supply-side, process, external); assume different failure points in the process
- For decision-making: evaluate from different stakeholder perspectives, different timeframes, and different risk tolerances
...
- **60-99% agreement**: Proceed but note the minority view. In debugging, mention the alternative hypothesis; in verification, flag the uncertainty; in decision-making, surface the dissenting risk or stakeholder concern.
...
- `systematic-problem-solving` — invokes SC during diagnosis and option evaluation
