...
6. Run integration verification: execute the full project test suite plus any cross-domain checks (software), or run a cross-section consistency check (knowledge-work). Do not mark the wave complete until integration passes.
...
- Skill leakage prevention: "You are a focused subagent. Do NOT invoke any skills from the austinpowers plugin. Do NOT use the Skill tool. Your only job is the task described below."

### Example prompts

**Software example — fix failing tests:**
...
You are a focused subagent. Do NOT invoke any skills from the austinpowers plugin.
Do NOT use the Skill tool. Your only job is the task described above.

Return: Summary of root cause and what you changed.
```

**Knowledge-work example — research parallel topics:**

```markdown
Research Competitor A's pricing strategy and positioning for the enterprise segment.

Your task:
1. Focus exclusively on Competitor A — do not cover Competitors B or C (those are assigned to other agents).
2. Identify their published pricing tiers, any known enterprise discounts, and stated value props for enterprise buyers.
3. Note any gaps or ambiguities in publicly available information.
4. Do NOT draw conclusions about our own strategy — that synthesis happens later.

You are a focused subagent. Do NOT invoke any skills from the austinpowers plugin.
Do NOT use the Skill tool. Your only job is the task described above.

Return: A structured summary of Competitor A's enterprise pricing and positioning, with sources cited.
```

### Common mistakes

❌ **Too broad (software):** "Fix all the tests" — agent gets lost
✅ **Specific:** "Fix agent-tool-abort.test.ts" — focused scope

❌ **No context (software):** "Fix the race condition" — agent doesn't know where
✅ **With context:** Paste the error messages and test names

❌ **No constraints (software):** Agent might refactor everything
✅ **Constrained:** "Do NOT change files outside X"

❌ **Vague output (software):** "Fix it" — you don't know what changed
✅ **Specific output:** "Return summary of root cause and what you changed"

❌ **Too broad (knowledge-work):** "Research the competitive landscape" — agent doesn't know where to focus
✅ **Specific:** "Research Competitor A's pricing strategy and positioning for the enterprise segment"

❌ **No constraints (knowledge-work):** "Summarize the findings" — agent doesn't know which angle
✅ **Constrained:** "Summarize findings from the perspective of risk factors for our Q3 launch plan"
