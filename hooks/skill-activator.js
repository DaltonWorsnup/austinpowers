 ...
 *
 * 1. Skill hints — which austinpowers skills are relevant to this prompt
 *    (reinforces using-austinpowers routing deterministically).
 *
...
function buildContext(matches) {
  ...
  const skillList = matches
    .map(m => `  - austinpowers:${m.skill} (${m.priority})`)
    .join('\n');

  return [
    '<user-prompt-submit-hook>',
    'Skill activation hint: The following skills are relevant to this prompt.',
    'Remember: invoke austinpowers:using-austinpowers FIRST as the mandatory entry point,',
