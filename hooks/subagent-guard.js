 ...
 *
 * Detects when subagents invoke austinpowers skills or spawn
...
const VIOLATION_PATTERNS = [
  /Invoke the austinpowers/i,
  /I'm using the .+ skill/i,
  /Skill\s*\(\s*["']?austinpowers/i,
  /skill:\s*["']?(austinpowers|using-austinpowers|brainstorming|deliberation|systematic-debugging|test-driven-development|verification|executing-plans|writing-plans|context-management|frontend-design|refactoring|performance-investigation|dependency-management)/i,
      ...
      for (const pattern of VIOLATION_PATTERNS) {
        if (pattern.test(lastMessage)) {
          ...
          const result = {
            decision: 'block',
            reason: [
              'SKILL LEAKAGE DETECTED: You invoked an austinpowers skill, which is not allowed for subagents.',
              'Redo your assigned task using only your core tools (Read, Edit, Write, Bash, Grep, Glob).',
              'Do NOT invoke the Skill tool. Do NOT reference any austinpowers skills.',
