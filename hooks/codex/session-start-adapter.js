...
const UPDATE_CACHE = path.join(getAustinpowersConfigDir(), 'update-check.cache');
...
function assembleUsingAustinpowers() {
  const skillPath = path.join(SKILLS_DIR, 'using-austinpowers', 'SKILL.md');
  const raw = readFileSafe(skillPath);
  if (!raw) return 'Error reading using-austinpowers skill';
